...
import logging
import time
from toolbox.json_functions import add_data_to_json, clean_json_file


def get_companies_raw_data() :
    '''
    get all the companies raw data
    '''
    # We get logging started
    logging.basicConfig(filename='app.log', filemode='w', format='%(name)s - %(levelname)s - %(message)s', level=logging.INFO)
    logging.info("////////////////////// get_companies_raw_data() //////////////////////")
    ...
    clean_json_file("../data/data_raw.json")  

    # We go through all the urls and get the raw data
    for i , url in enumerate(urls):
        logging.info(f" Pourcentage : {round(((i/len(urls))*100),2)} %  Company :{url}")
        print(f" Pourcentage : {round(((i/len(urls))*100),2)} %  Company n°{i+1} :{url}")

        # We get raw_data from company's url
        company_raw_data = scrap_company_raw_data(url)

        # We add the raw data to the json
        add_data_to_json("../data/data_raw.json", company_raw_data)
...
def scrap_company_raw_data(url):
    '''
    ...
    for i in range(int(pages_nbr)):
        ...
        for comment_div in comments_divs:
            ...
            # Extract the rating 
            rating = comment_div.find('div', class_ = "star-rating_starRating__4rrcf star-rating_medium__iN6Ty").img.get('alt').split(' ')[1]
            ...
            customer_location_exist = comment_div.find(class_ ="typography_body-m__xgxZ_ typography_appearance-subtle__8_H2l styles_detailsIcon__Fo_ua")
            if not customer_location_exist:
                customer_location =""
            else:
                customer_location =customer_location_exist.span.text.strip()
            # Extract the comment title
            comment_title = comment_div.find('h2' , class_ = 'typography_heading-s__f7029 typography_appearance-default__AAY17').text.strip()
